...
import { FiStar, FiMapPin, FiHeart, FiWifi, FiSun, FiCoffee } from 'react-icons/fi';
  ...
  return (
    ...
    >
      <div className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
        <Link to={`/property/${_id || id}`} className="block">
          <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
            ...
            <div className="absolute top-3 left-3 flex gap-2">
              <span className="bg-white/90 backdrop-blur-sm text-gray-900 text-xs font-bold px-3 py-1 rounded-full shadow-sm">{type}</span>
              <span className="bg-rushkey-500/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">{gender}</span>
            </div>
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2">
              <motion.button
                ...
                aria-label="Favorite"
              >
                <FiHeart className={`w-4 h-4 transition-colors ${favorited ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} />
              </motion.button>
            </div>

            <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-1.5 shadow-sm">
              <span className="text-rushkey-500 font-bold text-base">₹{displayPrice}</span>
          ...
          <div className="p-4 space-y-3">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-semibold text-gray-900 text-base leading-tight line-clamp-1 flex-1">{title}</h3>
            ...
            <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
              {amenities.slice(0, 4).map((amenity) => {
                ...
                return Icon ? (
                  <span key={amenity} className="flex items-center gap-1 text-xs text-gray-500 bg-gray-50 px-2.5 py-1.5 rounded-lg">
                    <Icon className="w-3.5 h-3.5" /> {amenity}
                  </span>
                ) : (
                  <span key={amenity} className="text-xs text-gray-500 bg-gray-50 px-2.5 py-1.5 rounded-lg">{amenity}</span>
                );
              })}
              {amenities.length > 4 && (
                <span className="text-xs text-gray-400 font-medium">+{amenities.length - 4}</span>
              )}
            </div>
          </div>
        </Link>
