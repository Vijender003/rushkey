...
import { HiOutlineUser, HiOutlineMail, HiOutlinePhone, HiOutlineLockClosed, HiEye, HiEyeOff } from 'react-icons/hi';
    ...
    >
      ...
      >
        ...
        <input
          ...
          placeholder={label}
          autoComplete={
            type === 'password' ? 'new-password' : type === 'email' ? 'email' : type === 'tel' ? 'tel' : 'name'
          }
        />
        <label
...
export default function SignupForm({ onSubmit, loading }) {
  ...
  const validate = () => {
    ...
    if (phone && !/^\+?[\d\s-]{10,15}$/.test(phone)) errs.phone = 'Please enter a valid phone number';
    if (!agreeTerms) errs.terms = 'Please agree to the terms';
    setErrors(errs);
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="text-center mb-4">
        ...
        <p className="text-gray-400 text-sm mt-1">Join Rushkey and find your perfect stay</p>
      </motion.div>
      ...
      <motion.div variants={itemVariants}>
        <label className="flex items-start gap-3 cursor-pointer group">
          <div className="relative mt-0.5">
            <input type="checkbox" checked={agreeTerms} onChange={(e) => { setAgreeTerms(e.target.checked); clearError('terms'); }} className="sr-only" />
            <div
              className={`w-4 h-4 rounded border-2 transition-all duration-200 flex items-center justify-center ${
                agreeTerms ? 'bg-rushkey-500 border-rushkey-500' : 'border-gray-300 group-hover:border-gray-400'
              }`}
            >
              {agreeTerms && (
          ...
          <span className="text-sm text-gray-500 group-hover:text-gray-700 transition-colors">
            I agree to the{' '}
            <button type="button" className="text-rushkey-500 hover:text-rushkey-600 font-medium">Terms of Service</button>
            {' '}and{' '}
            <button type="button" className="text-rushkey-500 hover:text-rushkey-600 font-medium">Privacy Policy</button>
          </span>
        </label>
